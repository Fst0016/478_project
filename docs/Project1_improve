...
typedef struct {
    ...
    int rts_sent;  // RTS frames sent
    int cts_received;  // CTS frames successfully received
} Station;
...
// Function to simulate CSMA/CA with VCS for a station
void simulate_csma_vcs(Station *station, int lambda, int topology) {
    int CW = CW0;
    ...
    while (time < SIMULATION_TIME * 1000000) {  // Convert seconds to microseconds
        ...
        if (time >= next_arrival) {
            ...
            if (station->backoff == 0) {
                ...
                station->rts_sent++;
                if (rand() % 100 >= 10) {  // Assuming 90% success rate for RTS/CTS
                    station->cts_received++;
...
int main() {
    ...
    for (int l = 0; l < NUM_LAMBDA_VALUES; l++) {
        ...
        for (int i = 0; i < NUM_STATIONS; i++) {
            ...
            stations[i].rts_sent = 0;
            stations[i].cts_received = 0;
        }
        
        // Simulate CSMA/CA with VCS for each station
        for (int i = 0; i < NUM_STATIONS; i++) {
            simulate_csma_vcs(&stations[i], lambda, topology);
        }
        ...
        for (int i = 0; i < NUM_STATIONS; i++) {
            printf("Station %d - Throughput: %.2f KBps, Collisions: %d, Frames Transmitted: %d, RTS Sent: %d, CTS Received: %d\n",
                   i + 1,
                   stations[i].throughput / 1000,  // Convert bytes to KB
                   stations[i].collisions,
                   stations[i].frames_transmitted,
                   stations[i].rts_sent,
                   stations[i].cts_received);
        }
